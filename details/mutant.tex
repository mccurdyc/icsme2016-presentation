\begin{tikzpicture}[node distance=0cm, auto,>=stealth, thick]
    ...
    \tikzstyle{smallbox}=[rectangle, draw=solarizedBase02, rounded corners, fill=solarizedBlue, drop shadow,
            ...
            \visible<1->{\node (op) [box] {a \textcolor{yellow}{$>$} \textcolor{white}{b}};}
            \visible<2->{\node (operator) [operator, below = 1cm of op] {Relational Operator Replacement (ROR)};}
            \visible<3->{\node (op1) [smallbox] at (-4.25cm, -4cm) {a $\geq$ b};}
            \visible<3->{\node (op2) [smallbox, right = 0.5cm of op1] {a $\leq$ b};}
            ...
            \visible<3->{\draw[->, line width = 3pt] (operator) -- (op3);}
            \visible<3->{\draw[->, line width = 3pt] (operator) -- (op4);}
